...
function initialize_simulation() {
    ...
    const community_distance_matrix = computeSimulatorObj.compute_community_distances(
        communities
    );
    let seed_array = [];
    if (SEEDING_MODE == SEED_INFECTION_RATES) {
        seed_array = initSimulatorObj.load_infection_seed_json(
            SEED_SCALING_FACTOR
        );
    }
    ...
    computeSimulatorObj.assign_individual_home_community(
        nodes,
        homes,
        workplaces,
        communities
    );
    computeSimulatorObj.assign_individual_public_transports(
        nodes,
        public_transports,
        workplaces
    );
    ...
    computeSimulatorObj.compute_scale_public_transport(
        nodes,
        public_transports
    );
...
function runSimulation() {
    ...
    let time_step = 0;
    time_step = computeObj.run_simday(
        time_step,
        homes,
        workplaces,
        communities,
    ...
    const interval = setInterval(function () {
        ...
        if (time_step >= NUM_TIMESTEPS || SIMULATION_STOP) {
            ...
            document.body.appendChild(link); // Required for FF
            document.getElementById("status").innerHTML =
                "Numbers plotted are per " + String(NUM_PEOPLE) + ".";
            if (!WEBPAGE_VERSION) {
                link.click(); //TODO: Instead of click link, add link for download on page.
            }
        }
...
function getData() {
    ...
    INTERVENTION = parseInt(
        document.querySelector('input[name="interventions2"]:checked').value
    );
