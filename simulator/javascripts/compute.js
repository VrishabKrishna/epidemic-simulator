...
compute.prototype.compute_community_distances = function (communities) {
    var inter_ward_distances_json = JSON.parse(
        loadJSON_001("input_files/wardCentreDistance.json")
    );
...
compute.prototype.assign_individual_home_community = function (
    nodes,
    homes,
    workplaces,
    communities
...
compute.prototype.assign_individual_public_transports = function (
    nodes,
    public_transports,
    workplaces
) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        if (
            ...
            nodes[i]["dist_hw"] = this.euclidean(
                nodes[i]["loc"],
...
compute.prototype.compute_scale_public_transport = function (
    nodes,
    public_transports
) {
...
compute.prototype.update_lambda_public_transport = function (
    nodes,
    public_transport
) {
    var sum_value = 0;
...
compute.prototype.update_lambda_c_global = function (
    communities,
    ...
    for (var c1 = 0; c1 < communities.length; c1++) {
        ...
        for (var c2 = 0; c2 < communities.length; c2++) {
            temp +=
                this.f_kernel(community_distance_matrix[c1][c2]) *
...
compute.prototype.update_lambdas = function (
    node,
    homes,
    workplaces,
    communities,
...
compute.prototype.update_all_kappa = function (
    nodes,
    homes,
    workplaces,
    communities,
    ...
    if (current_time < NUM_DAYS_BEFORE_INTERVENTIONS * SIM_STEPS_PER_DAY) {
        ...
        switch (INTERVENTION) {
            ...
            case LD_21_CI_HQ_SD70_SC_21:
                get_kappa_LD_21_CI_HQ_SD70_SC_21(
                    nodes,
                    homes,
                    workplaces,
                    communities,
            ...
            case LD_21_CI_HQ_SD70_SC_OE_30:
                get_kappa_LD_21_CI_HQ_SD70_SC_OE_30(
                    nodes,
                    homes,
                    workplaces,
                    communities,
...
compute.prototype.run_simday = function (
    time_step,
    homes,
    workplaces,
    communities,
    ...
    for (var h = 0; h < NUM_HOMES; h++) {
        homes[h]["age_dependent_mixing"] = this.update_lambda_h(
            nodes,
            homes[h]
        );
    }
    for (var w = 0; w < NUM_SCHOOLS + NUM_WORKPLACES; w++) {
        workplaces[w]["age_dependent_mixing"] = this.update_lambda_w(
            nodes,
            workplaces[w]
        );
    }
    ...
    for (var c = 0; c < NUM_COMMUNITIES; c++) {
        communities[c]["lambda_community"] = this.update_lambda_c_local(
            nodes,
            communities[c]
        );
    ...
    for (var pt = 0; pt < NUM_PUBLIC_TRANSPORT; pt++) {
        public_transports[pt][
            "lambda_PT"
        ] = this.update_lambda_public_transport(nodes, public_transports[pt]);
    }
