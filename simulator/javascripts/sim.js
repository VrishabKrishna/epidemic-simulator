...
BETA_W = 0.5; //0.47 *1.0//Thailand data
BETA_S = 1.0; //0.94 *1.0//Thailand data
...
function compute_prob_infection_given_community(
    infection_probability,
    set_uniform
) {
    var prob_infec_given_community = [];
    var communities_population_json = JSON.parse(
        loadJSON_001(inputPath + "fractionPopulation.json")
    );
    if (!set_uniform) {
        var communities_frac_quarantined_json = JSON.parse(
            loadJSON_001(inputPath + "quarantinedPopulation.json")
        );
    }
    ...
    for (var w = 0; w < num_communities; w++) {
        if (SEEDING_MODE == RANDOM_SEEDING_WARDWISE) {
            if (set_uniform) {
                ...
                prob_infec_given_community.push(
                    (infection_probability *
                        communities_frac_quarantined_json[w][
                            "fracQuarantined"
                        ]) /
                        communities_population_json[w]["fracPopulation"]
                );
            }
        } else {
...
function load_infection_seed_json(seed_scaling_factor) {
    var seed_array = [];
    var infection_seeds_json = JSON.parse(
        loadJSON_001(inputPath + "infection_seeds.json")
    );
    var infection_seeds_json_array = Object.values(
        infection_seeds_json["seed_fit"]
    );
    var num_seed_days = infection_seeds_json_array.length;
    for (var count = 0; count < num_seed_days * SIM_STEPS_PER_DAY; count++) {
        var mean =
            (infection_seeds_json_array[Math.floor(count / SIM_STEPS_PER_DAY)] /
                SIM_STEPS_PER_DAY) *
            seed_scaling_factor;
...
function infection_seeding_exp_rate(nodes, curr_time) {
    if (
        curr_time >= SEEDING_START_DATE * SIM_STEPS_PER_DAY &&
        curr_time < (SEEDING_START_DATE + SEEDING_DURATION) * SIM_STEPS_PER_DAY
    ) {
        var time_since_seeding_start =
            curr_time - SEEDING_START_DATE * SIM_STEPS_PER_DAY;
        var seed_doubling_time = SEEDING_DOUBLING_TIME * SIM_STEPS_PER_DAY;
        var current_seeding_rate =
            SEEDING_RATE_SCALE *
            Math.pow(2, time_since_seeding_start / seed_doubling_time);
...
function compute_scale_homes(homes) {
    for (var w = 0; w < homes.length; w++) {
        if (homes[w]["individuals"].length == 0) {
            ...
            homes[w]["scale"] =
                (BETA_H * homes[w]["Q_h"]) /
                Math.pow(homes[w]["individuals"].length, ALPHA);
        }
    }
}
...
function compute_scale_workplaces(workplaces) {
    ...
    for (var w = 0; w < workplaces.length; w++) {
        if (workplaces[w]["individuals"].length == 0) {
            ...
            workplaces[w]["scale"] =
                (beta_workplace * workplaces[w]["Q_w"]) /
                workplaces[w]["individuals"].length;
        }
    }
}
...
function compute_scale_communities(nodes, communities) {
    for (var w = 0; w < communities.length; w++) {
        ...
        if (sum_value == 0) {
            ...
            communities[w]["scale"] =
                (BETA_C * communities[w]["Q_c"]) / sum_value;
    }
}
...
function compute_scale_public_transport(nodes, public_transports) {
    for (var tr = 0; tr < public_transports.length; tr++) {
        ...
        for (var i = 0; i < public_transports[tr]["individuals"].length; i++) {
            sum_value +=
                nodes[public_transports[tr]["individuals"][i]]["dist_hw"];
        }
        if (sum_value == 0) {
            public_transports[tr]["scale"] = 0;
        } else {
            public_transports[tr]["scale"] =
                (BETA_PT * public_transports[tr]["Q_c"]) / sum_value;
        }
...
function init_workplaces() {
    var workplaces_json = JSON.parse(
        loadJSON_001(inputPath + "workplaces.json")
    );
...
function init_community() {
    var communities_json = JSON.parse(
        loadJSON_001(inputPath + "commonArea.json")
    );
...
function compute_community_distances(communities) {
    var inter_ward_distances_json = JSON.parse(
        loadJSON_001(inputPath + "wardCentreDistance.json")
    );

    var community_dist_matrix = math.zeros([
        communities.length,
        communities.length,
    ]);
    ...
    */
    ...
    for (var c1 = 0; c1 < inter_ward_distances_json.length; c1++) {
        for (var c2 = c1 + 1; c2 < inter_ward_distances_json.length; c2++) {
            community_dist_matrix[c1][c2] =
                inter_ward_distances_json[c1][(c2 + 1).toString()];
            community_dist_matrix[c2][c1] = community_dist_matrix[c1][c2];
        }
    }
...
function compute_individual_community_distance(nodes, communities) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        nodes[i]["funct_d_ck"] = f_kernel(
            euclidean(
                nodes[i]["loc"],
                communities[nodes[i]["community"]]["loc"]
            )
        );
    }
}

function assign_individual_home_community(
    nodes,
    homes,
    workplaces,
    communities
...
function assign_individual_public_transports(
    nodes,
    public_transports,
    workplaces
) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        if (
            nodes[i]["public_transport"] != null &&
            (nodes[i]["workplace_type"] == WTYPE_OFFICE ||
                nodes[i]["workplace_type"] == WTYPE_SCHOOL)
        ) {
            public_transports[nodes[i]["public_transport"]]["individuals"].push(
                i
            ); //No checking for null as all individuals have a home
            nodes[i]["dist_hw"] = euclidean(
                nodes[i]["loc"],
                workplaces[nodes[i]["workplace"]]["loc"]
            );
        }
    }
}
...
function get_init_stats(nodes, homes, workplaces, communities) {
    for (var h = 0; h < homes.length; h++) {
        ...
        for (
            var ind_count = 0;
            ind_count < homes[h]["individuals"].length;
            ind_count++
        ) {
            if (
                nodes[homes[h]["individuals"][ind_count]]["workplace_type"] == 1
            )
                working_count++;
        }
        console.log(
    ...
    for (var h = 0; h < workplaces.length; h++) {
        console.log(
            ...
            h,
            " - ",
            workplaces[h]["individuals"].length
        );
    }
    for (var h = 0; h < communities.length; h++) {
        console.log(
            ...
            h,
            " - ",
            communities[h]["individuals"].length
        );
    }
}
...
function update_infection(node, cur_time) {
    ...
    if (
        node["infection_status"] == SUSCEPTIBLE &&
        Math.random() < 1 - Math.exp(-node["lambda"] / SIM_STEPS_PER_DAY)
    ) {
...
function get_age_dependent_mixing_matrix_household() {
    var sigma_json = JSON.parse(
        loadJSON_001(inputPath + "Sigma_household.json")
    );
    var U_matrix_json = JSON.parse(
        loadJSON_001(inputPath + "U_household.json")
    );
    var V_matrix_json = JSON.parse(
        loadJSON_001(inputPath + "Vtranspose_household.json")
    );
...
function update_lambda_h(nodes, home) {
    ...
    if (!USE_AGE_DEPENDENT_MIXING) {
        ...
        for (
            var indv_count = 0;
            indv_count < home["individuals"].length;
            indv_count++
        ) {
            var indv_age_group =
                nodes[home["individuals"][indv_count]]["age_group"];
            age_components[indv_age_group] +=
                nodes[home["individuals"][indv_count]]["lambda_h"];
        }
        //weighted sum of age contributions for each eigen component
        var V_T_x = math.zeros([SIGNIFICANT_EIGEN_VALUES]);
        for (
            var eigen_count = 0;
            eigen_count < SIGNIFICANT_EIGEN_VALUES;
            eigen_count++
        ) {
            for (var count = 0; count < NUM_AGE_GROUPS; count++) {
                V_T_x[eigen_count] +=
                    V_MATRIX[eigen_count][count] * age_components[count]; //Assumption is V_matrix is V' where C = USV'
            }
        }
        ...
        for (var count = 0; count < NUM_AGE_GROUPS; count++) {
            for (
                ...
                lambda_age_group[count] +=
                    home["scale"] *
                    SIGMA[eigen_count] *
                    U_MATRIX[count][eigen_count] *
                    V_T_x[eigen_count];
            }
        }
    }
...
function get_age_dependent_mixing_matrix_workplace() {
    var sigma_json = JSON.parse(
        loadJSON_001(inputPath + "Sigma_workplace.json")
    );
    var U_matrix_json = JSON.parse(
        loadJSON_001(inputPath + "U_workplace.json")
    );
    var V_matrix_json = JSON.parse(
        loadJSON_001(inputPath + "Vtranspose_workplace.json")
    );
    ...
    var V_matrix_json = JSON.parse(
        loadJSON_001(inputPath + "Vtranspose_school.json")
    );
    ...
    for (var count = 0; count < NUM_AGE_GROUPS; count++) {
        //TODO:Change loop max to NUM_AGE_GROUPS
...
function update_lambda_w(nodes, workplace) {
    ...
    if (!USE_AGE_DEPENDENT_MIXING) {
        ...
        for (
            var student_count = 0;
            student_count < workplace["individuals"].length;
            student_count++
        ) {
            var student_age_group =
                nodes[workplace["individuals"][student_count]]["age_group"];
            age_components[student_age_group] +=
                nodes[workplace["individuals"][student_count]]["lambda_w"];
        }
        //weighted sum of age contributions for each eigen component
        var V_T_x = math.zeros([SIGNIFICANT_EIGEN_VALUES]);
        for (
...
function update_lambda_c_global(communities, community_distance_matrix) {
    for (var c1 = 0; c1 < communities.length; c1++) {
        ...
        for (var c2 = 0; c2 < communities.length; c2++) {
            temp +=
                f_kernel(community_distance_matrix[c1][c2]) *
...
function get_infected_community(nodes, community) {
    ...
    for (var i = 0; i < community["individuals"].length; i++) {
        ...
        if (
            nodes[community["individuals"][i]]["infection_status"] == RECOVERED
        ) {
            recovered_stat += 1;
        }
        if (
            nodes[community["individuals"][i]]["infection_status"] ==
            HOSPITALISED
        ) {
            hospitalised_stat += 1;
        }
...
function update_lambdas(
    ...
    if (node["home"] != null && node["home"] != undefined) {
        ...
        node["lambda_incoming"][0] =
            node["kappa_H_incoming"] *
            homes[node["home"]]["age_dependent_mixing"][age_group];
    }
    if (node["workplace"] != null && node["workplace"] != undefined) {
        var age_group = node["age_group"];
        node["lambda_incoming"][1] =
            node["kappa_W_incoming"] *
            workplaces[node["workplace"]]["age_dependent_mixing"][age_group];
    }
    ...
    if (
        node["public_transport"] != null &&
        node["public_transport"] != undefined
    ) {
        //for all public transport
        //console.log("In lambda incoming.")
        node["lambda_incoming"][3] =
            node["kappa_PT_incoming"] *
            node["dist_hw"] *
            public_transports[node["public_transport"]]["lambda_PT"];
    }
...
function update_lambda_stats(node) {
    ...
    for (var i = 0; i < node["lambda_incoming"].length; i++) {
        ...
        LAMBDA_INFECTION_MEAN[i] =
            (LAMBDA_INFECTION_MEAN[i] * LAMBDA_INFECTION_STATS.length +
                lambda_vector[i]) /
            (LAMBDA_INFECTION_STATS.length + 1);
    }
    ...
    if (current_time < NUM_DAYS_BEFORE_INTERVENTIONS * SIM_STEPS_PER_DAY) {
        ...
        switch (INTERVENTION) {
            ...
            case CASE_ISOLATION_AND_HOME_QUARANTINE_SD_70_PLUS:
                ...
                break;
            case LOCKDOWN_40_CI_HQ_SD_70_PLUS_21_CI:
                get_kappa_LOCKDOWN_40_CI_HQ_SD_70_PLUS_21_CI(
                    nodes,
                    homes,
                    workplaces,
                    communities,
            ...
            case LOCKDOWN_40:
                ...
                break;
            case LD_40_CI_HQ_SD70_SC_21_SC_42:
                get_kappa_LD_40_CI_HQ_SD70_SC_21_SC_42(
                    nodes,
                    homes,
                    workplaces,
                    communities,
                ...
                break;
            case LD_40_CI_HQ_SD70_SC_21:
                get_kappa_LD_40_CI_HQ_SD70_SC_21(
                    nodes,
                    homes,
                    workplaces,
                    communities,
                ...
                break;
            case LD_40_CI_HQ_SD70_SC_OE_30:
                get_kappa_LD_40_CI_HQ_SD70_SC_OE_30(
                    nodes,
                    homes,
                    workplaces,
                    communities,
...
function run_simday(
    ...
    for (var w = 0; w < NUM_SCHOOLS + NUM_WORKPLACES; w++) {
        workplaces[w]["age_dependent_mixing"] = update_lambda_w(
            nodes,
            workplaces[w]
        );
    }
    ...
    for (var c = 0; c < NUM_COMMUNITIES; c++) {
        communities[c]["lambda_community"] = update_lambda_c_local(
            nodes,
            communities[c]
        );
    ...
    for (var pt = 0; pt < NUM_PUBLIC_TRANSPORT; pt++) {
        public_transports[pt]["lambda_PT"] = update_lambda_public_transport(
            nodes,
            public_transports[pt]
        );
    }
    ...
    csvContent_ninfected =
        [time_step / SIM_STEPS_PER_DAY, n_infected].join(",") + "\r\n";
    ...
    if (LAMBDA_INFECTION_STATS.length > 0) {
        lambda_evolution.push([
            time_step / SIM_STEPS_PER_DAY,
            [
                LAMBDA_INFECTION_MEAN[0],
                LAMBDA_INFECTION_MEAN[1],
                LAMBDA_INFECTION_MEAN[2],
                LAMBDA_INFECTION_MEAN[3],
            ],
        ]);
    }
...
function run_simulation() {
    ...
    const interval = setInterval(function () {
        ...
        if (time_step >= NUM_TIMESTEPS || SIMULATION_STOP) {
            ...
            document.getElementById("status").innerHTML =
                "Numbers plotted are per " + String(numKs) + ".";
            if (!WEBPAGE_VERSION) {
                link.click(); //TODO: Instead of click link, add link for download on page.
            }
        }
...
function plot_lambda_evolution(data, plot_position, title_text, legends) {
    ...
    for (var count = 0; count < data.length; count++) {
        for (
            var lambda_length_count = 0;
            lambda_length_count < data[0][0][1].length;
            lambda_length_count++
        ) {
...
function call_plotly(data_tuple) {
    ...
    plot_plotly(
        [plot_values[2]],
        "num_hospitalised_plot_2",
        "Number Hospitalised (daily)",
        "Evolution of Hospitalised Population"
    );
    plot_plotly(
        [plot_values[3]],
        "num_critical_plot_2",
        "Number Critical (daily)",
        "Evolution of Critical Population"
    );
    plot_plotly(
        [plot_values[4]],
        "num_fatalities_plot_2",
        "Number Fatalities (cum.)",
        "Evolution of Fatalities Population"
    );
    //plot_plotly([plot_values[5]],'num_recovered_plot_2','Number Recovered (cum.)','Evolution of Recovered Population');
    plot_plotly(
        [plot_values[6]],
        "num_affected_plot_2",
        "Number Symptomatic (cum.)",
        "Evolution of Affected Population"
    );
...
function runSimulations() {
    ...
    INIT_FRAC_INFECTED =
        parseFloat(document.getElementById("initFrac").value) / 100.0;
    //	COMPLIANCE_PROBABILITY = document.getElementById("compliance").value;

    INCUBATION_PERIOD =
        parseFloat(document.getElementById("Incubation").value) / 2;
    INCUBATION_PERIOD_SCALE = INCUBATION_PERIOD * SIM_STEPS_PER_DAY; // 2.29 days

    MEAN_ASYMPTOMATIC_PERIOD = document.getElementById("asymptomaticMean")
        .value;
    ...
    MEAN_HOSPITAL_REGULAR_PERIOD = document.getElementById("meanHospitalPeriod")
        .value;
    MEAN_HOSPITAL_CRITICAL_PERIOD = document.getElementById("meanICUPeriod")
        .value;
    COMPLIANCE_PROBABILITY = document.getElementById("compliance").value;
    ...
    HOSPITAL_REGULAR_PERIOD = MEAN_HOSPITAL_REGULAR_PERIOD * SIM_STEPS_PER_DAY;
    HOSPITAL_CRITICAL_PERIOD =
        MEAN_HOSPITAL_CRITICAL_PERIOD * SIM_STEPS_PER_DAY;
...
function set_default_values_html() {
    ...
    document.getElementById(
        "asymptomaticMean"
    ).value = MEAN_ASYMPTOMATIC_PERIOD;
    ...
    document.getElementById(
        "meanHospitalPeriod"
    ).value = MEAN_HOSPITAL_REGULAR_PERIOD;
    document.getElementById(
        "meanICUPeriod"
    ).value = MEAN_HOSPITAL_CRITICAL_PERIOD;
    ...
    if (window.matchMedia("(max-width: 767px)").matches) {
        fetch("simulator/html/body_mobile.html")
            ...
            .then(() => {
                let h =
                    $(window).outerHeight() -
                    ($(".custom-header").outerHeight() +
                        $(".sticky-footer").outerHeight());
                $(".content-mobile, .left-pane, .right-pane").css(
                    "height",
                    h - 25
                );
...
function initListner() {
    $("#numDays").tooltip({
        trigger: "focus",
        title: "Number of days to run the simulation",
    });
    $("#Incubation").tooltip({
        trigger: "focus",
        title:
            "Once exposed to the virus, the mean duration before the exposed individual starts transmitting the virus.",
    });

    $("#asymptomaticMean").tooltip({
        trigger: "focus",
        title:
            "The mean duration for which asymptomatic or presymptomatic individuals may transmit the virus before recovering or before symptoms begin to show.",
    });

    $("#symptomaticMean").tooltip({
        trigger: "focus",
        title:
            "If symptoms begin to show, the mean duration for which an infected individual exhibits symptoms and continues to transmit the virus.",
    });

    $("#symtomaticFraction").tooltip({
        trigger: "focus",
        title: "The fraction of the exposed individuals that show symptoms.",
    });

    $("#meanHospitalPeriod").tooltip({
        trigger: "focus",
        title:
            "Some symptomatic individuals may need hospital care. If admitted, the mean duration of their stay in a regular ward.",
    });

    $("#meanICUPeriod").tooltip({
        trigger: "focus",
        title:
            "Some hospitalised individuals may develop complications, and may need intensive care. If so, the mean duration of their stay in an intensive care unit.",
    });

    $("#betaHouse").tooltip({
        trigger: "focus",
        title:
            "The expected number of eventful (infection spreading) contact opportunities in a day at home.",
    });

    $("#betaWork").tooltip({
        trigger: "focus",
        title:
            "The expected number of eventful contact opportunities in a day at the workplace.",
    });

    $("#betaSchools").tooltip({
        trigger: "focus",
        title:
            "The expected number of eventful contact opportunities in a day at the school.",
    });

    $("#betaCommunity").tooltip({
        trigger: "focus",
        title:
            "The expected number of eventful contact opportunities in the community interaction space.",
    });

    $("#betaPT").tooltip({
        trigger: "focus",
        title:
            "The expected number of eventful contact opportunities in the transport interaction space.",
    });

    $("#initFrac").tooltip({
        trigger: "focus",
        title:
            "What percentage of population is exposed to the virus? The residual incubation period is random.",
    });

    $("#compliance").tooltip({
        trigger: "focus",
        title:
            "What fraction of households are likely to follow the restrictions specified in the chosen intervention?",
    });
...
$(window).resize(function () {
    if (!$("body").hasClass("mobile")) {
        let h =
            $(window).outerHeight() -
            ($(".custom-header").outerHeight() + $(".footer").outerHeight());
        $(".splitter-container").css("height", h);

        let h1 =
            $(".splitter-container").outerHeight() -
            $(".run-btn-cnt").outerHeight();
        $(".scroll").css("height", h1);

        let h2 =
            $(".splitter-container").outerHeight() -
            $(".right-top").outerHeight();
